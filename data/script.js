(function () {
    ...
    let socket;
    const espUrl = 'http://esp.local';
    // const espUrl = window.location.hostname;
                ...
                switch (theClass[1]) {
                    case 'debugPrint':
                        if (socket) {
                            ...
                            elem.innerText = "Stop";
                            activeOn(elem);
                            socket = new WebSocket(`ws://${espUrl}/data`);
                            socket.onmessage = e => {
    ...
    async function scanNetworks() {
        ...
        while (!data) {
            try {
                const response = await fetch(espUrl + "/wifiNetworks");
    ...
    function sendData(name, data1, data2) {
        fetch(`${espUrl}/updateData?output=${name}&state=${data1}${data2 !== undefined ? `&state2=${data2}` : ''}`);
    }

    function getState() {
        fetch(espUrl + '/getState')
            .then(e => e.json())
            .then(e => {
                espData = e;
                refreshPageData(0);
